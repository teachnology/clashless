...
def test_chair_is_own_participant_is_scheduled_validly():
...
def test_shared_supervisor_forces_split_across_days():
    # p1 and p2 share participant Bob Jones and there's only one session per day,
    # so they cannot share a day. Carol White (p1's participant_3) is unavailable
...
def test_participant_is_grouped_into_fewest_possible_days():
    # Nora Chen chairs p1-p4, Omar Reyes chairs p5-p8; they share one
    # participant (Priya Shah, on p1 and p5), which is just enough cross-
    # contention that a non-optimizing solver can end up interleaving the two
    # chairs across 3-4 of the 4 available days instead of cleanly giving
...
def test_few_presentations_are_scheduled_back_to_back():
    # Eve Davis chairs p1-p3, Frank Ito chairs p4-p6; each of Eve's
    # presentations shares its participant_2 with one of Frank's (Priya Shah,
...
def test_large_synthetic_dataset_produces_a_valid_schedule():
    ...
    # chairs (one chairs up to ~53 presentations). n_days=10 (80 slots at 8
    # sessions/day) gives that bottleneck chair enough room; unavailable.csv's
...
def test_optimize_grouping_false_skips_the_objective_and_solves_fast():
    ...
    # chairs, each pairwise sharing a participant with the next) - with the
...
def _total_active_days(schedule, presentations):
    merged = schedule.join(presentations.data)
    return merged.groupby("chair")["day"].nunique().sum()


def test_grouping_is_never_worse_than_not_grouping():
    # On several_competing_moderators, the grouping objective's auxiliary
    # variables used to make the model large enough that CP-SAT could spend most
    # of the time budget just reaching feasibility - sometimes landing on a
    # worse arrangement than simply not optimizing at all would have found by
    # chance. solve() now warm-starts the objective search from a plain
    # feasibility-only solution, so the grouped result can only be as good or
    # better, never worse. Run several times since both solves are individually
    # non-deterministic (CP-SAT's parallel search).
    scenario = load_scenario("several_competing_moderators")
    ...
    for _ in range(3):
        ...
        ungrouped_total = _total_active_days(ungrouped, scenario.presentations)
        grouped_total = _total_active_days(grouped, scenario.presentations)
